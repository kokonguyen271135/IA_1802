...
class CPEExtractor:
    ...
    KNOWN_PATTERNS = {
        ...
        'riot client services':  ('riot_games', 'riot_client'),
        'riot client':           ('riot_games', 'riot_client'),
        'riotclientservices':    ('riot_games', 'riot_client'),
        'riotclient':            ('riot_games', 'riot_client'),
        'riot games':            ('riot_games', 'riot_client'),
        'valorant':              ('riot_games', 'valorant'),
        'league of legends':     ('riot_games', 'league_of_legends'),
    ...
    def _match_name(self, name: str, company: str) -> tuple:
        ...
        for keyword in sorted(self.KNOWN_PATTERNS, key=len, reverse=True):
            if self._keyword_matches_text(keyword, name):
                return self.KNOWN_PATTERNS[keyword]

        if company:
            for keyword in sorted(self.KNOWN_PATTERNS, key=len, reverse=True):
                if self._keyword_matches_text(keyword, company):
                    return self.KNOWN_PATTERNS[keyword]

            for co_kw, vendor in self.COMPANY_TO_VENDOR.items():
                if self._keyword_matches_text(co_kw, company):
    ...
    def _compact_text(self, text: str) -> str:
        return re.sub(r'[^a-z0-9]+', '', self._normalize_text(text))
    ...
    def _keyword_matches_text(self, keyword: str, text: str) -> bool:
        ...
        if kw_norm in text_norm:
            # Avoid tiny substring false positives such as "git" in "logitech".
            if ' ' not in kw_norm and len(kw_norm) <= 3:
                return kw_norm in text_norm.split()
            return True

        kw_compact = self._compact_text(keyword)
        text_compact = self._compact_text(text)
        if not kw_compact or not text_compact:
            return False

        if len(kw_compact) <= 3:
            return kw_compact in {self._compact_text(tok) for tok in text_norm.split()}
