...
Use an XGBoost model trained on EMBER 2017 (600K PE samples) to
compute malware probability for a PE file from 1390 static features.

This is real ML-based AI scoring — replaces the heuristic rule-based
scoring in PEStaticAnalyzer._calculate_risk().
...
def score_file(file_path: str) -> dict:
    """
    Run EMBER XGBoost inference on a single PE file.
