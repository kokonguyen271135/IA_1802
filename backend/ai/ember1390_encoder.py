...
====================
Feature extractor for PE files — matches EXACTLY the training pipeline in V3.0
...
# =========================
# NUMPY COMPAT PATCH
# Old ember uses np.int / np.bool / np.float...
# Use a warnings filter to suppress FutureWarning when doing hasattr checks on deprecated aliases.
# =========================
import warnings
...
def extract_raw_features_from_exe(exe_path: str) -> Dict[str, Any]:
    if ember is None:
        raise ImportError(f"Could not import the ember library: {EMBER_IMPORT_ERROR}")
    ...
    try:
        ...
        raise RuntimeError(
            f"Could not extract raw features from the PE file. "
            f"The file may be invalid, or ember/lief/numpy versions may be incompatible. "
            f"Details: {e}"
        ) from e
...
# =========================
# VECTORIZE — matches train_ember2017_full_gpu.py :: vectorize_ember_raw()
# =========================
