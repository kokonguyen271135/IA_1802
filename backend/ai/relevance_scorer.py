...
Use SecBERT to compute semantic similarity between the behavior profile of a PE
file and the CVE description. This is the only way relevance is computed —
there is no longer any rule-based scoring with hardcoded weights.

When SecBERT is not available:
  → Do not fake a score; just sort CVEs by CVSS score (highest first).
