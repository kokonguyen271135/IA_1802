...
function getSeverityRank(cve) {
    const severity = cve.bert_prediction?.predicted_severity
        || cve.zero_shot_prediction?.predicted_severity
        || cve.ml_prediction?.predicted_severity
        || cve.severity
        || 'NONE';
    return { CRITICAL: 4, HIGH: 3, MEDIUM: 2, LOW: 1, NONE: 0 }[severity] || 0;
}

function sortCvesByPriority(vulnerabilities) {
    return [...vulnerabilities].sort((a, b) => {
        if ((b.known_exploited ? 1 : 0) !== (a.known_exploited ? 1 : 0)) {
            return (b.known_exploited ? 1 : 0) - (a.known_exploited ? 1 : 0);
        }
        if ((b.has_public_exploit ? 1 : 0) !== (a.has_public_exploit ? 1 : 0)) {
            return (b.has_public_exploit ? 1 : 0) - (a.has_public_exploit ? 1 : 0);
        }
        const severityDiff = getSeverityRank(b) - getSeverityRank(a);
        if (severityDiff !== 0) return severityDiff;
        const cvssDiff = (parseFloat(b.cvss_score) || 0) - (parseFloat(a.cvss_score) || 0);
...
function displayResults(data) {
    ...
    const sorted = sortCvesByPriority(data.vulnerabilities);
    sorted.forEach(cve => {
...
function renderAiPanel(ai, panelId, riskBadgeId, summaryId, threatsId, recsId, vectorsId, decisionId) {
    ...
    if (!ai || !ai.success) {
        panel.style.display = 'none';
        renderPatchDecision(null, decisionId);
        return;
    }
    ...
    renderPatchDecision(ai.update_decision, decisionId);

    // ── CLEAN: hide threats/recs, show safe-state notice ───────────────────
...
function renderPatchDecision(decision, containerId) {
    ...
    const color = PATCH_ACTION_COLORS[decision.action] || '#2563eb';
    const reasons = (decision.reasons || [])
        .map(item => `<li>${escapeHtml(item)}</li>`)
        .join('');
    const reviewChecks = (decision.review_checks || [])
        .map(item => `<li>${escapeHtml(item)}</li>`)
        .join('');
    const topCves = (decision.top_cves || [])
        .map(item => {
            const fix = item.fixed_version_label || item.fixed_version || 'Review advisory';
            const badges = [
                item.known_exploited ? '<span class="patch-mini-badge kev">KEV</span>' : '',
                item.has_public_exploit ? '<span class="patch-mini-badge exploit">PoC</span>' : '',
            ].join('');
            return `
                <div class="patch-cve-item">
                    <div class="patch-cve-title">
                        <span>${escapeHtml(item.cve_id || 'N/A')}</span>
                        <span class="patch-cve-meta">CVSS ${escapeHtml(String(item.cvss_score || 'N/A'))}</span>
                    </div>
                    <div class="patch-cve-badges">${badges}</div>
                    <div class="patch-cve-fix">Fix target: ${escapeHtml(fix)}</div>
                </div>`;
        })
        .join('');
    const cia = decision.cia || {};
    const ciaText = `C:${cia.confidentiality || 'NONE'} / I:${cia.integrity || 'NONE'} / A:${cia.availability || 'NONE'}`;
    const recommendedVersion = decision.recommended_version_label || decision.recommended_version || 'Review vendor advisory';
    const avgCvss = Number(decision.avg_cvss || 0).toFixed(1);
    const compatibilityChecks = (decision.compatibility_checks || [])
        .map(item => `
            <div class="patch-check-item">
                <div class="patch-check-top">
                    <strong>${escapeHtml(item.title || '')}</strong>
                    <span class="patch-check-status">${escapeHtml(item.status || 'CHECK')}</span>
                </div>
                <div class="patch-check-detail">${escapeHtml(item.detail || '')}</div>
            </div>
        `)
        .join('');

    container.innerHTML = `
        <div class="patch-decision-card" style="border-color:${color}">
            <div class="patch-decision-top">
                <div>
                    <div class="patch-decision-label">Patch Decision</div>
                    <div class="patch-decision-action" style="color:${color}">${escapeHtml(decision.action)}</div>
                </div>
                <div class="patch-decision-priority" style="background:${color}">${escapeHtml(decision.priority || '')}</div>
            </div>
            <p class="patch-decision-summary">${escapeAndBreakSentences(decision.summary || '')}</p>
            <div class="patch-metric-grid">
                <div class="patch-metric">
                    <span class="patch-metric-label">Recommended Fix</span>
                    <strong>${escapeHtml(recommendedVersion)}</strong>
                </div>
                <div class="patch-metric">
                    <span class="patch-metric-label">Known Exploited</span>
                    <strong>${escapeHtml(String(decision.kev_count || 0))}</strong>
                </div>
                <div class="patch-metric">
                    <span class="patch-metric-label">Public PoC</span>
                    <strong>${escapeHtml(String(decision.public_exploit_count || 0))}</strong>
                </div>
                <div class="patch-metric">
                    <span class="patch-metric-label">CIA Impact</span>
                    <strong>${escapeHtml(ciaText)}</strong>
                </div>
                <div class="patch-metric">
                    <span class="patch-metric-label">Avg CVSS</span>
                    <strong>${escapeHtml(avgCvss)}</strong>
                </div>
                <div class="patch-metric">
                    <span class="patch-metric-label">Remote / No Auth</span>
                    <strong>${escapeHtml(String(decision.remote_no_auth_count || 0))}</strong>
                </div>
            </div>
            ${reasons ? `
                <div class="patch-section">
                    <h5>Why This Decision</h5>
                    <ul class="patch-list">${reasons}</ul>
                </div>` : ''}
            ${topCves ? `
                <div class="patch-section">
                    <h5>Priority CVEs</h5>
                    <div class="patch-cve-list">${topCves}</div>
                </div>` : ''}
            ${compatibilityChecks ? `
                <div class="patch-section">
                    <h5>Compatibility & Deployment Checks</h5>
                    <div class="patch-check-grid">${compatibilityChecks}</div>
                </div>` : ''}
            ${reviewChecks ? `
                <div class="patch-section">
                    <h5>Pre-Upgrade Checks</h5>
                    <ul class="patch-list">${reviewChecks}</ul>
                </div>` : ''}
        </div>`;
...
//     // Bonus: click to view full description
//     if (description.length > 250) {
//         div.querySelector('.cve-description').addEventListener('click', () => {
//             alert(description); // or open a nicer modal
//         });
...
function normalizeImpactLabel(label) {
    ...
    if (value === 'COMPLETE') return 'HIGH';
    if (value === 'PARTIAL') return 'MEDIUM';
    if (value === 'NONE') return 'NONE';
    return value || 'NONE';
}

function formatCiaTriplet(cve) {
    const c = normalizeImpactLabel(cve.confidentiality_impact);
    const i = normalizeImpactLabel(cve.integrity_impact);
    const a = normalizeImpactLabel(cve.availability_impact);
    return `C:${c} / I:${i} / A:${a}`;
}

function getFixVersionLabel(cve) {
    return cve.fixed_version_label || cve.fixed_version || cve.suggested_fix_version || '';
}

function getExploitStatus(cve) {
    if (cve.known_exploited) {
        const kevDate = cve.cisa_kev && cve.cisa_kev.date_added ? ` since ${cve.cisa_kev.date_added}` : '';
        return `Known exploited in the wild (CISA KEV${kevDate})`;
    }
    if (cve.has_public_exploit) {
        return 'Public exploit / PoC referenced by NVD';
    }
    return 'No public exploit referenced in NVD';
}

function isNetworkReachable(cve) {
    const attackVector = String(cve.attack_vector || '').toUpperCase();
    const vectorString = String(cve.vector_string || '').toUpperCase();
    return attackVector.includes('NETWORK') || vectorString.includes('AV:N');
}

function isNoAuthRequired(cve) {
    const privs = String(cve.privileges_required || '').toUpperCase();
    const vectorString = String(cve.vector_string || '').toUpperCase();
    return privs === 'NONE' || privs === 'N' || vectorString.includes('PR:N') || vectorString.includes('AU:N');
}

function needsUserAction(cve) {
    const interaction = String(cve.user_interaction || '').toUpperCase();
    const vectorString = String(cve.vector_string || '').toUpperCase();
    return interaction === 'REQUIRED' || interaction === 'R' || vectorString.includes('UI:R');
}

function buildIntelBadges(cve) {
    const badges = [];
    if (cve.known_exploited) badges.push('<span class="intel-badge kev">CISA KEV</span>');
    if (!cve.known_exploited && cve.has_public_exploit) badges.push('<span class="intel-badge exploit">Public PoC</span>');
    if (isNetworkReachable(cve)) badges.push('<span class="intel-badge remote">Remote</span>');
    if (isNoAuthRequired(cve)) badges.push('<span class="intel-badge noauth">No Auth</span>');
    if (needsUserAction(cve)) badges.push('<span class="intel-badge user">User Action</span>');
    badges.push(`<span class="intel-badge cia">${escapeHtml(formatCiaTriplet(cve))}</span>`);
    return badges.join('');
}

function buildIntelMetaRows(cve) {
    const rows = [];
    if (cve.affected_range) rows.push(['Affected range', cve.affected_range]);
    const fix = getFixVersionLabel(cve);
    if (fix) rows.push(['Recommended fix', fix]);
    rows.push(['Exploit status', getExploitStatus(cve)]);
    if (cve.published) rows.push(['Published', String(cve.published).substring(0, 10)]);

    return rows.map(([label, value]) => `
        <div class="cve-meta-item">
            <span class="cve-meta-label">${escapeHtml(label)}</span>
            <span class="cve-meta-value">${escapeHtml(value)}</span>
        </div>
    `).join('');
}
...
function createCVEItem(cve) {
    ...
    const intelBadges = buildIntelBadges(cve);
    const intelMetaRows = buildIntelMetaRows(cve);
    ...
    div.innerHTML = `
        ...
        <div class="cve-intel-row">${intelBadges}</div>
        <p class="cve-summary">${escapeHtml(shortDesc)}</p>
        <div class="cve-meta-grid">${intelMetaRows}</div>
        <div class="cve-links">
...
function formatStaticSignalFactor(staticRisk, finalLevel, stats, aiRisk) {
    const hasScore = typeof staticRisk.score !== 'undefined';
    const score = Math.round(Number(staticRisk.score) || 0);
    const staticLevel = staticRisk.level || '';
    if (!hasScore && !staticLevel) return null;

    const finalRank = getRiskRank(finalLevel);
    const staticRank = getRiskRank(staticLevel);
    const cveCount = Number(stats.total_cves) || 0;
    const emberLevel = aiRisk.level || '';

    if (staticRank > finalRank && finalRank <= getRiskRank('LOW')) {
        const reasons = [];
        if (cveCount === 0) reasons.push('0 applicable CVEs');
        if (emberLevel && getRiskRank(emberLevel) <= getRiskRank('LOW')) {
            reasons.push(`EMBER stayed ${emberLevel}`);
        }

        let detail = `Static-only score: ${score} / 100`;
        if (reasons.length > 0) {
            detail += `; final assessment remained ${finalLevel} because ${reasons.join(' and ')}.`;
        } else {
            detail += `; raw PE heuristics were downgraded in the final assessment.`;
        }
        return detail;
    }

    return `Static-only score: ${score} / 100` +
        (staticLevel ? ` (${staticLevel})` : '');
}

function buildPERiskBannerData(data) {
    const aiAnalysis = data.ai_analysis || {};
    const stats = data.cve_statistics || {};
    const cpe = data.cpe || '';
    const cwePredicted = ((data.cwe_analysis || {}).predicted_cwes) || [];

    if (aiAnalysis.success) {
        const finalLevel = aiAnalysis.overall_risk || 'CLEAN';
        const factors = [
            'Assessment prioritizes confirmed CVE matches and resolved software/version evidence.',
        ];
        if (cpe) {
            factors.push(`Resolved software evidence: ${cpe}`);
        } else {
            factors.push('Software identity could not be resolved from the file metadata.');
        }
        if (cwePredicted.length > 0) {
            factors.push('Behavioral CWE hints are shown separately and are not counted as confirmed CVE matches.');
        }
        factors.push(
            `Confirmed CVEs: ${Number(stats.total_cves) || 0} | Avg CVSS: ${Number(stats.avg_cvss || 0).toFixed(1)}`
        );

        return {
            level: finalLevel,
            subtitle: 'Overall Match Assessment',
            factors,
        };
    }
    ...
    return {
        level: 'CLEAN',
        subtitle: 'Overall Match Assessment',
        factors: ['No confirmed CVE match identified yet.'],
    };
}
...
function renderPEResults(data) {
    ...
    renderRiskBanner(buildPERiskBannerData(data));
    renderPECVEs(data);
    renderComponents(data.components);
    const codebertCard = document.getElementById('codebertCard');
    const secbertCard = document.getElementById('secbertProfileCard');
    if (codebertCard) codebertCard.style.display = 'none';
...
function _renderPECVEList(listEl, vulns, stats, title) {
    ...
    const sorted = sortCvesByPriority(vulns);
...
function _truncateText(text, maxLength = 120) {
...
function _appendPECWEBadges(listEl, cweAnalysis) {
    const cwePredicted = (cweAnalysis && cweAnalysis.predicted_cwes) || [];
    if (cwePredicted.length === 0) return;

    const block = document.createElement('div');
    block.className = 'cwe-prediction-block';

    const badgeHtml = cwePredicted.map(c => {
        const color = CODEBERT_SEVERITY_COLORS[c.label] || '#9ca3af';
        return `<span class="cwe-prediction-badge" style="background:${color};">
            ${escapeHtml(c.cwe_id || c.label)}</span>`;
    }).join('');

    const cardHtml = cwePredicted.slice(0, 6).map(c => {
        const color = CODEBERT_SEVERITY_COLORS[c.label] || '#9ca3af';
        const name = c.name || c.cwe_id || 'Unknown CWE';
        const description = _truncateText(c.description || '', 150);
        const confidence = typeof c.confidence === 'number'
            ? `${Math.round(c.confidence * 100)}% confidence`
            : '';
        const triggerPreview = _truncateText((c.triggered_by || []).slice(0, 2).join(' | '), 150);

        return `
        <div class="cwe-prediction-card" style="border-left-color:${color};">
            <div class="cwe-prediction-card-top">
                <span class="cwe-prediction-id" style="background:${color};">${escapeHtml(c.cwe_id || '')}</span>
                ${confidence ? `<span class="cwe-prediction-confidence">${escapeHtml(confidence)}</span>` : ''}
            </div>
            <div class="cwe-prediction-name">${escapeHtml(name)}</div>
            ${description ? `<div class="cwe-prediction-desc">${escapeHtml(description)}</div>` : ''}
            ${triggerPreview ? `<div class="cwe-prediction-trigger">Signals: ${escapeHtml(triggerPreview)}</div>` : ''}
        </div>`;
    }).join('');

    block.innerHTML = `
        <div class="cwe-prediction-title">
            <strong>Predicted Weakness Themes (CWE):</strong>
            <span class="cwe-prediction-subtitle">Exploratory weakness categories inferred from binary behavior.</span>
        </div>
        <div class="cwe-prediction-badges">${badgeHtml}</div>
        <div class="cwe-prediction-grid">${cardHtml}</div>`;
    listEl.appendChild(block);
}

function _appendBehavioralCVESuggestions(listEl, data) {
    const suggestions = data.behavioral_cve_suggestions || [];
...
function renderPECVEs(data) {
    ...
    const aiPanel     = document.getElementById('peAiPanel');

    if (aiPanel) aiPanel.style.display = 'none';
    renderPatchDecision(null, 'pePatchDecision');
    ...
    const meta = [
        ...
        cpeInfo.confidence ? `Confidence: ${String(cpeInfo.confidence).toUpperCase()}` : '',
    ...
    if (vulns.length === 0) {
        ...
        listEl.innerHTML = `<p style="color:#10b981; padding:10px;">
            <i class="fas fa-check-circle"></i>&nbsp;No confirmed CVEs found for this resolved software version.
        </p>`;
        _appendPECWEBadges(listEl, data.cwe_analysis || {});
        _appendBehavioralCVESuggestions(listEl, data);
        return;
    }
    ...
    listEl.innerHTML = '';
    _appendPECWEBadges(listEl, data.cwe_analysis || {});
    _renderPECVEList(listEl, vulns, stats, 'Matched CVEs');
    _appendBehavioralCVESuggestions(listEl, data);
}
...
function renderCodeBERTAnalysis(cb) {
    ...
    // ── SecBERT profile text (hidden — profile is too generic to display) ──
    if (secbertCard) secbertCard.style.display = 'none';
...
function renderRiskBanner(risk) {
    ...
    document.getElementById('riskScore').textContent = risk.subtitle
        || (isAI ? `AI Risk Score: ${score} / 100` : `Risk Score: ${score} / 100`);
...
// Escape and split text into separate lines by sentence.
// Splits on ". " / "! " / "? " when followed by a capital letter, and on "; ".
// Numeric periods (e.g. "7.7", "9.0.277.0+") and abbreviations mid-sentence
// are preserved because the next character there is a digit, not a capital letter.
function escapeAndBreakSentences(str) {
    if (str == null) return '';
...
function showCVEDetailModal(cve) {
        ...
        modal.innerHTML = `
            <div class="modal-content">
                ...
                <div class="modal-meta">
                    ...
                    <p><strong>Exploit Status:</strong> <span id="modal-exploit-status"></span></p>
                    <p><strong>Affected Range:</strong> <span id="modal-affected-range"></span></p>
                    <p><strong>Recommended Fix:</strong> <span id="modal-fix-version"></span></p>
                    <p><strong>CIA Impact:</strong> <span id="modal-cia"></span></p>
                </div>
                <div id="modal-kev-box" class="modal-intel-box" style="display:none;">
                    <h3><i class="fas fa-satellite-dish"></i> CISA KEV</h3>
                    <p id="modal-kev-text"></p>
                </div>
    ...
    document.getElementById('modal-exploit-status').textContent = getExploitStatus(cve);
    document.getElementById('modal-affected-range').textContent = cve.affected_range || 'Not specified by NVD';
    document.getElementById('modal-fix-version').textContent = getFixVersionLabel(cve) || 'Review vendor advisory';
    document.getElementById('modal-cia').textContent = formatCiaTriplet(cve);
    document.getElementById('modal-description').textContent = cve.description || 'No description from NVD.';

    const kevBox = document.getElementById('modal-kev-box');
    const kevText = document.getElementById('modal-kev-text');
    if (cve.known_exploited && cve.cisa_kev) {
        const due = cve.cisa_kev.due_date ? ` Action due: ${cve.cisa_kev.due_date}.` : '';
        const action = cve.cisa_kev.required_action ? ` ${cve.cisa_kev.required_action}` : '';
        kevText.textContent = `Added to CISA KEV on ${cve.cisa_kev.date_added || 'N/A'}.${due}${action}`.trim();
        kevBox.style.display = 'block';
    } else {
        kevBox.style.display = 'none';
        kevText.textContent = '';
    }
    ...
    const refDetails = cve.reference_details || [];
    const allRefs = refDetails.length > 0 ? refDetails.map(ref => ref.url) : (cve.references || []);
    const maxShow = 5;
    const visibleRefs = allRefs.slice(0, maxShow).map((url, index) => refDetails[index] || { url });
    const hiddenRefs  = allRefs.slice(maxShow).map((url, index) => refDetails[maxShow + index] || { url });
    const renderRef = (ref) => {
        const tags = (ref.tags || []).map(tag => `<span class="modal-ref-tag">${escapeHtml(tag)}</span>`).join('');
        return `
            <div class="modal-ref-item">
                <a href="${escapeHtml(ref.url)}" target="_blank">${escapeHtml(ref.url)}</a>
                ${tags ? `<div class="modal-ref-tags">${tags}</div>` : ''}
            </div>`;
    };
    refsDiv.innerHTML = visibleRefs.map(renderRef).join('') + (hiddenRefs.length > 0 ? `
        <div id="refs-hidden" style="display:none">${hiddenRefs.map(renderRef).join('')}</div>
...
// Close modal from close button or outside click
function closeModal() {
