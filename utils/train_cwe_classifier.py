...
Input  : CVE description text (or PE behavior profile text)
Output : CWE category (CWE-78, CWE-94, CWE-119, ...)

Why this model?
  Track 3 needs to predict CWEs from a PE file's behavior text.
  Pipeline:
    PE static analysis → build_profile_text() → [this model] → CWE → NVD query

How it works:
  1. Train on CVE descriptions → CWE labels (NVD ground truth)
  2. At inference: feed PE behavior text into the model → predict CWE
  3. Assumption: a PE's behavior text is close to the language of CVE descriptions
     (same cybersecurity domain → SecBERT can handle it)

Dataset: data/training/cve_cwe_train.csv
  - Generated automatically by build_training_data.py
  - Columns: cve_id, description, cwe_ids, severity
  - cwe_ids: pipe-separated, e.g. "CWE-94|CWE-78"
  - Use the first CWE (primary CWE) as the class label
...
# Maximum number of CWE classes to train on.
# Use the top-N most common CWEs from NVD to avoid long-tail noise.
DEFAULT_TOP_CWES = 20

# CWEs that actually appear in PE/native binary vulnerabilities.
# Drop web-only ones: CWE-79 (XSS), CWE-89 (SQLi), CWE-22 (path traversal),
# CWE-352 (CSRF), CWE-918 (SSRF), CWE-611 (XXE).
PE_RELEVANT_CWES = [
    "CWE-119",  # Buffer Overflow (includes stack/heap)
...
def load_dataset(path: Path, top_n: int = DEFAULT_TOP_CWES) -> tuple[list, dict, dict, int]:
    """
    Load cve_cwe_train.csv and select PE-relevant CWEs (whitelist).

    Instead of top-N by frequency (easily dominated by web CWEs like XSS/SQLi),
    use the PE_RELEVANT_CWES whitelist to make sure the model learns the right domain.

    Deduplicates by cve_id to avoid train/val/test leakage from oversampling.
    ...
    skipped   : number of records that were dropped
    """
    ...
    with open(path, encoding="utf-8") as f:
        ...
        for row in reader:
            ...
            # Take the primary CWE (first one in the list)
    ...
    # ── Use the PE_RELEVANT_CWES whitelist instead of top-N by frequency ──
    # Keep only whitelisted CWEs that have enough samples in the dataset
